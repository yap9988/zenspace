...
//v1.5 - notifcation toggle & color tune
//v1.6 - color tune
...
input color ExtColor6 = Green;  // Bull candlestick body
input int EMA_Period = 20;  // EMA period
input int InpRSIPeriod = 14;  // RSI Period
input bool AlertM = False;
...
int init() { 
   ...
   SetIndexStyle(11,DRAW_LINE, 0, 2, ExtColor6);
   SetIndexArrow(11,119);
   SetIndexStyle(12,DRAW_LINE, 0, 2, ExtColor3);
...
int start() { 
 ...
 //     if( ExtRSIBuffer[pos+2] < ExtRSIBufferSMA[pos+2] && ExtRSIBuffer[pos+1] > ExtRSIBufferSMA[pos+1])
      ...
      if (ExtTotalScore[1]==High[1] && Close[0] > High[1]&& now > expiryTimeArraybear && AlertM)
      {
      ...
      if (ExtTotalScore[1]==Low[1] && Close[0] < Low[1]&& now > expiryTimeArraybull && AlertM)
      {
...
//    string alertMessage3 = Symbol() + ": The Bull chance has come (OB)";
   ...
   if ( ExtDownFractalsBuffer[1]==High[1])
   {
   if (ExtRSIDownFractalsBuffer[1]==High[1]*1.01)
   {
    showvalue1 = "Bear+";
   }
   else
   ...
   if (ExtUpFractalsBuffer[1]==Low[1])
   {
   if (ExtRSIUpFractalsBuffer[1]==Low[1]*0.99)
   {
    showvalue1 = "Bull+";
   }
   else

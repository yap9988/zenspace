...
//handles the alerts
void doAlertFVG(string symbol,string timeFrameName,string pivotLevelName, int bb)//handles all the alert messages
  {
            ...
            if(alertModeSelection==Cross_Alerts ||
               alertModeSelection==Cross_And_Bounce_Alerts)
              {
               ...
               if(alertModeSelection==Near_Alerts ||
                 ...
                 {
                        PivotsWaitTill[symbolIndex][j]=(TimeCurrent()+alertIntervalTimeSeconds);
                        doAlertFVG(symbolName,ppTimeFrame,pivotNamesRef[j],1); //needtorevisethis
                 }
                 if (pivotSelection==FVG && TimeCurrent()>=PivotsWaitTill[symbolIndex][j]&& FVGCheck[symbolIndex][0]<0) //something logic checking here, try revising trdacheck/calculatetrda
                 {
                        PivotsWaitTill[symbolIndex][j]=(TimeCurrent()+alertIntervalTimeSeconds);

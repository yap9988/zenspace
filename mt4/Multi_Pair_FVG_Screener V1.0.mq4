...
void calculatekama(ENUM_TIMEFRAMES timeFrame,double &FVGArrayRef[][],int symbolIndex,string symbolName)
 {
  ...
  for (int j = 1000; j > 1; j--)
    ...
    for (int  h = 0; h<i-2; h++)
    {
    if (iClose(symbolName,0,h)>tempup)
    {
    ...
    for (int  g = 0; g<i-2; g++)
    {
    if (iClose(symbolName,0,g)<temp2down)
    {
    ...
    SendNotification(alertMessage);
       ...
       FVGArrayRef[symbolIndex][1]=bearOBdown[symbolIndex][f];
    }
    ...
    SendNotification(alertMessage1);
       FVGArrayRef[symbolIndex][0]=-1;
       FVGArrayRef[symbolIndex][1]=bullOBup[symbolIndex][f];

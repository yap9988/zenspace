...
//+------------------------------------------------------------------+
//v1.0 the first working version, where notification is made when fvg formed (confirm) and in the last 24 candles in 5M chart, there is a crossover in price_close and 20SMA
//v1.1 add KAMA in, use kama to define to trade bull or bear, last fvg whether being break, if yes and match kama, send signal
//v1.2 fixed repeating signals, if same fvg need not to send again every 15 mins in V1.1; fixed invalid bear signal for fvg1,2 equals to 0
//v1.3 send kama & strength info, let user to check the trend and decide the trade
//v1.4 further fine tune good & bad indication; added last low3 / high3 for FVG breakage, also fix a very major issue only defining good bad (inverse)
//v1.5 wording fine tune, stable version
//v1.6 detection from 15M to 5M
//v1.7 change iclose to ilow & ihigh for FVG break detection, need not to close below/above it for considering as LQ grab; change scoring calculation mechanism in doalertfvg1
//Alpha version, switch kama detection & add (C) on notification message
//kama v1.0 - kama cross 15M chart
//kama v1.1 - get daily kama value & change the arrow definition (V^) in the message
//kama v1.1A - 1H version instead of 15M
//kama v1.2A - 1H version, to prematurely give signal instead of waiting another candle for confirmation
    ...
    else
    {
      ...
      stt=stt1+stt2;
       ...
       //  string messageAlert=symbol+" FVG "+pType+". "+firstc+": "+firstS[symbolIndex][0]+st1+", "+lastc+": "+secondS[symbolIndex][0]+st2+". "+"Score: "+stt+". V: "+power+". "; last known good config
          ...
          string messageAlert=symbol+pType+"(1H)"+"D: "+pTypeD+"|S:"+stt+"|"+firstc+":"+firstS[symbolIndex][0]+st1+","+lastc+":"+secondS[symbolIndex][0]+st2+"|V:"+power;
...
void calculatekama(ENUM_TIMEFRAMES timeFrame,double &FVGArrayRef[][],int symbolIndex,string symbolName)
 {
   ...
   double kamau1,kamau2,kamau3,kamad1,kamad2,kamad3;
   double FVG1=1000, FVG2=1000, FVG3=-1000, FVG4=-1000;
   int nearestFVG1 = -1, nearestFVG2 = -1;
   int nearestFVG3 = -1, nearestFVG4 = -1;
    //8 ExtDownFractalsBuffer[1] == High[1]
    //7 ExtUpFractalsBuffer[1] == Low[1]
     kamau1=iCustom(symbolName,PERIOD_H1,"Limaocanfly\\KAMA V1.6",7,1);
     kamau2=iCustom(symbolName,PERIOD_H1,"Limaocanfly\\KAMA V1.6",7,2);
    // kamau3=iCustom(symbolName,PERIOD_H1,"Limaocanfly\\KAMA V1.6",7,3);
     kamad1=iCustom(symbolName,PERIOD_H1,"Limaocanfly\\KAMA V1.6",8,1);
     kamad2=iCustom(symbolName,PERIOD_H1,"Limaocanfly\\KAMA V1.6",8,2);
    // kamad3=iCustom(symbolName,PERIOD_H1,"Limaocanfly\\KAMA V1.6",8,3);
    ...
    /*
    if(kamau1 == iLow(symbolName, PERIOD_M15, 1)&& kamau2 == iLow(symbolName, PERIOD_M15, 2) && kamad3 == iHigh(symbolName, PERIOD_M15, 3))
    {
    Print(symbolName,"Kamau1 is ilow1, kamau2 is ilow2, kamad3 is ihigh3");
    }
    
       
    
    if(kamau3 == iLow(symbolName, PERIOD_M15, 3) && kamad2 == iHigh(symbolName, PERIOD_M15, 2) && kamad1 == iHigh(symbolName, PERIOD_M15, 1))
    {
    Print(symbolName,"Kamau1 is ilow1, kamau2 is ilow2, kamad3 is ihigh3, ivernse");
    }
    */
    ...
    if (kamad2 == iHigh(symbolName, PERIOD_H1, 2) && kamau1 == iLow(symbolName, PERIOD_H1, 1) )
    {
           FVGArrayRef[symbolIndex][0]=1;
           FVGArrayRef[symbolIndex][1]=iLow(symbolName, PERIOD_H1, 1);
    }
    
    
    
     
    if (kamau2 == iLow(symbolName, PERIOD_H1, 2) && kamad1 == iHigh(symbolName, PERIOD_H1, 1) )
    {
           FVGArrayRef[symbolIndex][0]=-1;
           FVGArrayRef[symbolIndex][1]=iHigh(symbolName, PERIOD_H1, 1);
    }
    ...
    /*
     ...
     if (kama1 == iLow(symbolName, PERIOD_D1, 1)) //kama bull
     {
   
     for (int i = 3; i < Bars - 1; i++) 
       {       
        if (iClose(symbolName, PERIOD_M15, i+1) < iLow(symbolName, PERIOD_M15, i+2) && iHigh(symbolName, PERIOD_M15, i) < iLow(symbolName, PERIOD_M15, i+2)) 
            {
            para1 = iHigh(symbolName, PERIOD_M15, i+2);         // The low of the current bar (i)
            // Store the first nearest FVG if it's not set yet
            if (nearestFVG1 == -1) {
                FVG1 = para1;           // Store the first FVG low
                nearestFVG1 = i;          // Store the index of the first FVG
            }
            // Store the second nearest FVG if it's the second one found
            else if (nearestFVG2 == -1) {
                FVG2 = para1;           // Store the second FVG low
                nearestFVG2 = i;          // Store the index of the second FVG
                break;                    // Stop the loop once two FVGs are found
            }
            }
        }
    if ( iHigh(symbolName, PERIOD_M5, 1)> FVG1 )
     {
       FVGArrayRef[symbolIndex][0]=1;
       FVGArrayRef[symbolIndex][1]=FVG1;
     }
     ...
     */
         ...
         /*
     if (kama2 == iHigh(symbolName, PERIOD_D1, 1))  //kama bear
     {
   
     for (int i = 3; i < Bars - 1; i++) 
       {       
        if (iClose(symbolName, PERIOD_M15, i+1) > iHigh(symbolName, PERIOD_M15, i+2) && iLow(symbolName, PERIOD_M15, i) > iHigh(symbolName, PERIOD_M15, i+2)) 
           {
            para1 = iLow(symbolName, PERIOD_M15, i+2);         // The low of the current bar (i)
            // Store the first nearest FVG if it's not set yet
            if (nearestFVG3 == -1) {
                FVG3 = para1;           // Store the first FVG low
                nearestFVG3 = i;          // Store the index of the first FVG
            }
            // Store the second nearest FVG if it's the second one found
            else if (nearestFVG4 == -1) {
                FVG4 = para1;           // Store the second FVG low
                nearestFVG4 = i;          // Store the index of the second FVG
                break;                    // Stop the loop once two FVGs are found
            }
            }
        }
    if ( iLow(symbolName, PERIOD_M5, 1)< FVG3 )
     {
       FVGArrayRef[symbolIndex][0]=-1;
       FVGArrayRef[symbolIndex][1]=FVG3;
     //  Print(symbolName,FVG3, FVG4,FVGArrayRef[symbolIndex][0]); 
     }   
               ...
               alertModeSelection==Cross_And_Bounce_Alerts)
              {
               ...
               if(alertModeSelection==Near_Alerts ||
                 ...
                 if (pivotSelection==FVG && TimeCurrent()>=PivotsWaitTill[symbolIndex][j] && FVGCheck[symbolIndex][0]>0 && FVGWaitTill[symbolIndex][j] != FVGCheck[symbolIndex][1]) //something logic checking here, try revising trdacheck/calculatetrda
                 {
                        ...
                        doAlertFVG1(symbolName,ppTimeFrame,pivotNamesRef[j],1,symbolIndex,FVGCheck[symbolIndex][1]); 
